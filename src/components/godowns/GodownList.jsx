...
export const GodownList = ({ dataService, currentUser }) => {
  ...
  return (
    <div>
      ...
      {/* Overall Summary */}
      <div className="godown-stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '24px' }}>
        {[
          ...
          <div key={stat.label} className="card godown-stat-card" style={{ padding: '16px', textAlign: 'center', background: stat.bg, border: `1px solid ${stat.color}22` }}>
            <div className="godown-stat-value" style={{ fontSize: '24px', fontWeight: 800, color: stat.color }}>{stat.value}</div>
            <div className="godown-stat-label" style={{ fontSize: '11px', color: '#64748b', marginTop: '4px', fontWeight: 600 }}>{stat.label}</div>
          </div>
        ))}
      </div>
